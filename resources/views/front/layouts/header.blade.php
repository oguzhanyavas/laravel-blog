...
<head>
    ...
    <title>@yield('title','Anasaysadsda') - {{ $configs->title }}</title>
...
<body>
    ...
    <nav class="navbar navbar-expand-lg navbar-light fixed-top" id="mainNav">
        <div class="container">
            ...
            <a class="navbar-brand" href="{{ route('homepage') }}">
                @if($configs->logo)
                <img src="{{ asset($configs->logo) }}" width="100" alt="">
                @else
                {{ $configs->title }}
                @endif
            </a>
    ...
    <header class="masthead" style="background-image: url('@yield('bg',asset(asset('front/img/home-bg.jpg')))');">
