    ...
    <div class="row">
        <div class="col-md-3">
            <div class="box-body no-padding">
                <a class="form-control" href="{{ route('admin.inbox.write') }}"><i class="fa fa-pencil"></i> Mesaj
                    Gönder</a>
                <a class="form-control text-primary" href="{{ route('admin.inbox.index') }}">
                    <i class="fa fa-inbox"></i> Gelen Kutusu <span class="text-bold pull-right">
                        @if($inboxCenter->count()>0){{ $inboxCenter->count() }}
                        @endif
                    </span>
                </a>
                <a class="form-control" href="{{ route('admin.inbox.sentedAll') }}"><i class="fa fa-envelope-o"></i>
                    Gönderilenler</a>
                <a class="form-control" href="{{ route('admin.inbox.trashed') }}"><i class="fa fa-trash-o"></i> Çöp
                    Kutusu </a>
            </div>
        </div>
        <!-- /.col -->
                ...
                <div class="box-body no-padding">
                    <form class="" action="{{ route('admin.replyMulti') }}" method="post">
                        ...
                        <div class="mailbox-controls">
                            ...
                            <div class="btn-group">
                                <button formaction="{{ route('admin.trashMulti') }}" type="submit"
                                    class="btn btn-default btn-sm"><i class="fa fa-trash-o"></i></button>

                                @csrf
                                <button type="submit" class="btn btn-default btn-sm"><i
                                        class="fa fa-reply"></i></button>
                                <button formaction="{{ route('admin.readedMulti') }}" type="submit"
                                    class="btn btn-default btn-sm"><i class="fa fa-check"></i> Okundu olarak
                                    işaretle</button>
                                <button formaction="{{ route('admin.unreadMulti') }}" type="submit"
                                    class="btn btn-default btn-sm"><i class="fa fa-envelope"></i> Okunmadı olarak
                                    işaretle</button>
                            </div>
                            <!-- /.btn-group -->
                        ...
                        <div class="table-responsive mailbox-messages">
                            <table class="table table-hover table-striped">
                                <tbody>
                                    ...
                                    @if (auth()->user()->email != $inbox->sEmail && auth()->user()->email ==
                                    $inbox->rEmail )
                                    ...
                                    <tr>
                                        ...
                                        <td class="mailbox-name"><a
                                                class="@if($inbox->status == 0) text-primary @else text-muted @endif"
                                                href="{{ route('admin.inbox.show',$inbox->id) }}">{{ $inbox->name }}</a>
                                        </td>
                                        <td class="mailbox-subject"><b>{{ $inbox->topic }}</b> -
                                            {{ Str::limit($inbox->message,80) }}
                                        </td>
